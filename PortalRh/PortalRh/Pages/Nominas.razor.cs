...
namespace PortalRh.Pages
{
    public partial class Nominas
    {
        ...
        private List<LineaModel> LineaModel;
        private string selectedDbTable;

        private string dbTable;
        private DateTime? fecPago;
        // Variable para guardar los valores separados de DbTable y FecPago
        private string dbTableValue;
        private string? fecPagoValue;
        ...
        {
            // Dividimos el valor que contiene tanto el DbTable como el fecPago
            var values = value.Split('|');
            if (values.Length == 4)
            {
                selectedDbTable = values[0]; // obtiene el DbTable
                qnaPago = values[1];
                fecPagoValue = values[2];    // obtiene fecPago formateado
                year = values[3];    // obtiene year formateado
            }
        }
        

        public async Task SipeSic()
        {
            var añonom = year;
            var qnaPa = qnaPago;
            var fechapago = fecPagoValue;
            string fec_pago2 = year + qnaPa.PadLeft(2, '0');
            DateTime fec_ini= DateTime.Parse(fecPagoValue);
            fec_ini =fec_ini.AddDays(-13);
            DateTime fec_pago_dt = DateTime.Parse(fecPagoValue);  // Convertir fec_pago a DateTime
            string fec_fin;
            // Da formato a la fecha en "yyyyMMdd"
            string fec_ini_formatted = fec_ini.ToString("yyyyMMdd");

            if (fec_ini.Day < 16)
            {
                // Sumar un día a fec_pago
                fec_fin = fec_pago_dt.AddDays(1).ToString("yyyyMMdd");
            }
            else
            {
                // Obtener el último día del mes
                DateTime lastDayOfMonth = new DateTime(fec_pago_dt.Year, fec_pago_dt.Month, DateTime.DaysInMonth(fec_pago_dt.Year, fec_pago_dt.Month));
                fec_fin = lastDayOfMonth.ToString("yyyyMMdd");
            }
            string fec_termi = fec_ini_formatted + fec_fin;
            var LineaModel = await myService.GetSipeSic(fec_ini.ToString("yyyyMMdd"),fec_pago2,selectedDbTable);
          
            var sb = new StringBuilder();
            
            

            // Añade cada elemento de LineaModel al StringBuilder
            foreach (var item in LineaModel)
            {
                sb.AppendLine($"{item.Linea}");
            }

            // Nombre del archivo
            var TxtName = "138" + qnaPa.ToString().PadLeft(2, '0') + year.ToString();

            // Convierte el contenido a un arreglo de bytes
            var byteArray = Encoding.UTF8.GetBytes(sb.ToString());

            // Invoca el servicio para descargar el archivo
        ...
        private async Task exportExcelTerceros()

        {
            ...
            var Metlife = await myService.GetResumenMetlife(selectedDbTable);
            var Kondinero = await myService.GetResumenKondinero(selectedDbTable);
            var Credifiel = await myService.GetResumenCredifiel(selectedDbTable);
            var Fonacot = await myService.GetResumenFonacot(selectedDbTable);
            var SutConalep = await myService.GetResumenSutConalep(selectedDbTable);
            var SitaCONQROO = await myService.GetResumenSITACONQROO(selectedDbTable);
            var Sitem = await myService.GetResumenSITEM(selectedDbTable);
            var Credifom = await myService.GetCrediFonReportModel(selectedDbTable);
            var AñoQuincena = selectedDbTable.Substring(12, 2) + "-" + selectedDbTable.Substring(7, 4);
            ...
            try
            {
                ...
                using (var book = new XLWorkbook(rutaPlantilla))
                {
                    ...
                    for (int row = 1; row <= list.Count; row++)
                    {
                        ...
                        sheet.Cell(row + 2, 12).Value = item.Credifom;
                    ...
                    for (int row = 1; row <= Metlife.Count; row++)
                    {
                        var item = Metlife[row - 1];
                    ...
                    for (int row = 1; row <= Kondinero.Count; row++)
                    {
                        var item = Kondinero[row - 1];
               ...
               for (int row = 1; row <= Credifiel.Count; row++)
                    {
                        var item = Credifiel[row - 1];
                    ...
                    for (int row = 1; row <= Fonacot.Count; row++)
                    {
                        var item = Fonacot[row - 1];
                    ...
                    for (int row = 1; row <= SutConalep.Count; row++)
                    {
                        var item = SutConalep[row - 1];
                    ...
                    for (int row = 1; row <= SitaCONQROO.Count; row++)
                    {
                        var item = SitaCONQROO[row - 1];
                    ...
                    for (int row = 1; row <= Sitem.Count; row++)
                    {
                        var item = Sitem[row - 1];
                    ...
                    IXLWorksheet Credifomsheet = book.Worksheet(9);

                    for (int row = 1; row <= Credifom.Count; row++)
                    {
                        var item = Credifom[row - 1];

                        Credifomsheet.Cell(row + 2, 1).Value = item.TIPO;
                        Credifomsheet.Cell(row + 2, 2).Value = item.CT;
                        Credifomsheet.Cell(row + 2, 3).Value = item.RFC;
                        Credifomsheet.Cell(row + 2, 4).Value = item.NOMBRE;
                        Credifomsheet.Cell(row + 2, 5).Value = item.Credifom;
                    }
                    ...
                    {
                        book.SaveAs(memory);
                        var nombreExcel = string.Concat("PagoTerceros_",AñoQuincena, ".xlsx");
        ...
        private async Task exportExcelFovisste()
        {
            if (string.IsNullOrEmpty(selectedDbTable))
            {
                Console.WriteLine("No se ha seleccionado ninguna tabla.");
            ...
            var list = await myService.GetFovissteReporte(selectedDbTable);

            try
            {
                var AñoQuincena = selectedDbTable.Substring(12, 2)+ "_" + selectedDbTable.Substring(7, 4)  ;
                // Ruta de la plantilla de Excel
                string rutaPlantilla = "wwwroot/Templates/ReporteFovissste.xlsx";
                ...
                using (var book = new XLWorkbook(rutaPlantilla))
                {
                    ...
                    for (int row = 1; row <= list.Count; row++)
                    {
                        ...
                        sheet.Cell(row + 2, 1).Value = item.TIPO;
                        sheet.Cell(row + 2, 2).Value = item.NOMBRE;
                        sheet.Cell(row + 2, 3).Value = item.RFC;
                        sheet.Cell(row + 2, 4).Value = item.SDO_BASE;
                        sheet.Cell(row + 2, 5).Value = item.FOVISSSTE;
                        sheet.Cell(row + 2, 6).Value = item.SEGURO_DE_DANOS;
                        sheet.Cell(row + 2, 7).Value = item.FOVISSSTE_CF;
                        sheet.Cell(row + 2, 8).Value = item.SDO_HON;
                    }
                    ...
                    {
                        book.SaveAs(memory);
                        var nombreExcel = string.Concat("ReporteFovissste_", AñoQuincena, ".xlsx");
        ...
        private async Task exportExcelFonacot()
        {
            if (string.IsNullOrEmpty(selectedDbTable))
            {
                Console.WriteLine("No se ha seleccionado ninguna tabla.");
            ...
            var list = await myService.GetFonacotReportModel(selectedDbTable);

            try
            {
                var AñoQuincena = selectedDbTable.Substring(12, 2) + "_" + selectedDbTable.Substring(7, 4);
                // Ruta de la plantilla de Excel
                string rutaPlantilla = "wwwroot/Templates/ReporteFonacot.xlsx";
                ...
                using (var book = new XLWorkbook(rutaPlantilla))
                {
                    ...
                    for (int row = 1; row <= list.Count; row++)
                    {
                        ...
                        sheet.Cell(row + 2, 3).Value = item.RFC;
                        sheet.Cell(row + 2, 4).Value = item.SDO_BASE;
                        sheet.Cell(row + 2, 5).Value = item.IMPORTE;
                    }
                    ...
                    {
                        book.SaveAs(memory);
                        var nombreExcel = string.Concat("ReporteFonacot_", AñoQuincena, ".xlsx");
        ...
        private async Task ExportarTxtSericaSPrestamos()
        {
            ...
            try
            {
                ...
                foreach (var item in header)
                    ...
                    sb.AppendLine($"{item.ENC0}|{item.ENC1 + AñoQuincena}|{item.FechaPago}|{item.ENC1}|{item.ENC2}|{item.ENC2}|{item.ENC2}|{item.IMPORTE_11301}|{item.IM12201}|{item.IM12301}|{item.IM13101}|{item.IMPORTE_13102}|{item.IM13401}|{item.IM13402}|{item.IM13407}|{item.IM13408}|{item.IM13411}|{item.IMPORTE_15403}|{item.IMPORTE_15402}|{item.despensa}|{item.prestamos.ToString("F0")}|{item.superissste}|{item.ade_medico.ToString("F0")}|{item.CHC}|{item.pension}|{item.faltas}|{item.retardos}|{item.TOT_PERC}|{item.tot_dedu}|{item.tot_neto}");
                ...
                foreach (var item in list)
                {
                    ...
                    sb.AppendLine($"{item.TI}|{item.NSS}|{item.NOMBRE}|{item.APE_PAT}|{item.APE_MAT}|{item.RFC}|{item.CURP}|{item.SEXO}|{item.PAGADURIA}|{item.NO_EMPLE}|{item.NUM_CHEQ}|{item.REGIMEN_ISSSTE}|{item.TIPO_CONTRATO}|{item.TOT_PERC.ToString("F2")}|{item.TOT_DEDU1.ToString("F2")}|{item.PARTIDA_11301}|{item.CONCEPTO_11301}|{item.IMPORTE_11301.ToString("F2")}|{item.PT12201}|{item.CP12201}|{item.IM12201}|{item.PT12301}|{item.CP12301}|{item.IM12301}|{item.PT13101}|{item.CP13101}|{item.IM13101}|{item.PARTIDA_13102}|{item.CONCEPTO_13102}|{item.IMPORTE_13102.ToString("F2")}|{item.IPT13401}|{item.ICP13401}|{item.IM13401}|{item.IPT13402}|{item.ICP13402}|{item.IM13402}|{item.IPT13407}|{item.ICP13407}|{item.IM13407}|{item.IPT13408}|{item.ICP13408}|{item.IM13408}|{item.IPT13411}|{item.ICP13411}|{item.IM13411}|{item.PARTIDA_15403}|{item.CONCEPTO_15403}|{item.IMPORTE_15403.ToString("F2")}|{item.PARTIDA_15402}|{item.CONCEPTO_15402}|{item.IMPORTE_15402.ToString("F2")}|{item.PARTIDA_10001}|{item.CONCEPTO_10001}|{item.IMPORTE_10001.ToString("F2")}|{item.PARTIDA_10002}|{item.CONCEPTO_10002}|{item.IMPORTE_10002.ToString("F2")}|{item.PARTIDA_20001}|{item.CONCEPTO_20001}|{item.IMPORTE_20001.ToString("F2")}|{item.PARTIDA_20002}|{item.CONCEPTO_20002}|{item.IMPORTE_20002.ToString("F2")}|{item.PT20003}|{item.CP20003}|{item.IM20003}|{item.PT20004}|{item.CP20004}|{item.IM20004}|{item.PARTIDA_20005}|{item.CONCEPTO_20005}|{item.IMPORTE_20005.ToString("F2")}|{item.PARTIDA_20006}|{item.CONCEPTO_20006}|{item.IMPORTE_20006.ToString("F2")}|{item.PARTIDA_20007}|{item.CONCEPTO_20007}|{item.IMPORTE_20007.ToString("F2")}");
